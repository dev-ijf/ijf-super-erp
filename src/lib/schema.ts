...
export const coreSettings = pgTable(
  ...
  {
    id: serial('id').primaryKey(),
    schoolId: integer('school_id').references(() => coreSchools.id),
  ...
  (table) => [
    uniqueIndex('unique_setting_per_school').on(table.schoolId, table.settingKey),
  ]
);
...
export const coreStudents = pgTable('core_students', {
  ...
  address: text('address'),
  provinceId: integer('province_id').references(() => coreProvinces.id),
  cityId: integer('city_id').references(() => coreCities.id),
  districtId: integer('district_id').references(() => coreDistricts.id),
  subdistrictId: integer('subdistrict_id').references(() => coreSubdistricts.id),
  postalCode: varchar('postal_code', { length: 10 }),
  phone: varchar('phone', { length: 20 }),
  ...
  // Health History
  bloodType: varchar('blood_type', { length: 2 }), // 'A' | 'B' | 'AB' | 'O'
...
// ==============================================================================
// 5. CORE: PARENT_STUDENT_RELATIONS
// ==============================================================================
export const coreParentStudentRelations = pgTable(
  ...
  (table) => [primaryKey({ columns: [table.userId, table.studentId] })]
);
...
export const coreClasses = pgTable('core_classes', {
  ...
  levelGradeId: integer('level_grade_id')
    .notNull()
    .references(() => coreLevelGrades.id),
  name: varchar('name', { length: 50 }).notNull(),
});

// ==============================================================================
// 8. CORE: STUDENT_CLASS_HISTORIES
// ==============================================================================
...
// ==============================================================================
// 11. TUITION: PAYMENT INSTRUCTION GROUPS
// ==============================================================================
export const tuitionPaymentInstructionGroups = pgTable(
  'tuition_payment_instruction_groups',
  {
    id: serial('id').primaryKey(),
    paymentMethodId: integer('payment_method_id')
      .notNull()
      .references(() => tuitionPaymentMethods.id),
    title: varchar('title', { length: 100 }).notNull(),
  }
);

// ==============================================================================
// 11B. TUITION: PAYMENT INSTRUCTION STEPS
// ==============================================================================
export const tuitionPaymentInstructionSteps = pgTable(
  'tuition_payment_instruction_steps',
  {
    id: serial('id').primaryKey(),
    groupId: integer('group_id')
      .notNull()
      .references(() => tuitionPaymentInstructionGroups.id),
...
export const tuitionBills = pgTable('tuition_bills', {
  ...
  status: varchar('status', { length: 10 }).default('unpaid'), // 'paid' | 'unpaid' | 'partial'
  relatedMonth: date('related_month'),
  createdAt: timestamp('created_at').defaultNow(),
});

// ==============================================================================
// 13. TUITION: TRANSACTIONS
// ==============================================================================
export const tuitionTransactions = pgTable('tuition_transactions', {
  ...
  qrCode: text('qr_code'),
  status: varchar('status', { length: 10 }).default('pending'), // 'success' | 'pending' | 'failed'
  paymentDate: timestamp('payment_date'),
  createdAt: timestamp('created_at').defaultNow(),
});

// ==============================================================================
// 14. TUITION: TRANSACTION DETAILS
// ==============================================================================
export const tuitionTransactionDetails = pgTable('tuition_transaction_details', {
  id: serial('id').primaryKey(),
  transactionId: integer('transaction_id')
    .notNull()
    .references(() => tuitionTransactions.id),
...
export const notifTemplates = pgTable('notif_templates', {
  ...
  schoolId: integer('school_id').references(() => coreSchools.id),
  name: varchar('name', { length: 100 }).notNull(),
  type: varchar('type', { length: 10 }).notNull(), // 'whatsapp' | 'email'
...
export const notifLogs = pgTable('notif_logs', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => coreUsers.id),
  templateId: integer('template_id').references(() => notifTemplates.id),
  type: varchar('type', { length: 10 }).notNull(), // 'whatsapp' | 'email'
  recipient: varchar('recipient', { length: 100 }).notNull(),
  requestPayload: text('request_payload'),
  responsePayload: text('response_payload'),
  status: varchar('status', { length: 10 }).default('pending'), // 'success' | 'failed' | 'pending'
  createdAt: timestamp('created_at').defaultNow(),
});
